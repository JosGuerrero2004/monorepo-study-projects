...
const Tarea: FC<ITareaProps> = ({ tarea, index }) => {
  const { onEliminar, onFinalizar } = useTareasContext()
  return (
    <li key={index.toString()}>
      <div className='task-header'>
        <h3>{tarea.nombre}</h3>
        <div className={`task-status ${tarea.estado?.toLowerCase()}`}>{tarea.estado}</div>
      </div>
      <p>
        <strong>Descripción:</strong> {tarea.descripcion}
      </p>
      <p>
        <strong>Fecha de vencimiento:</strong> {tarea.fecha.toUTCString()}
      </p>
