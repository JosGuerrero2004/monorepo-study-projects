...
export function tareasReducer(state: TareasState, action: TareasAction): TareasState {
  switch (action.type) {
    case 'CARGAR_TAREAS':
      return {
        ...state,
        tareas: action.payload.map((tarea: ITarea) => {
          return {
            ...tarea,
    ...
    case 'FINALIZAR_TAREA':
      return {
        ...
        tareas: state.tareas.map((tarea) =>
          tarea.id === action.payload ? { ...tarea, estado: 'Finalizado' } : tarea
        ),
      }
      case 'ELIMINAR_TAREA':
        return {
